 ...
 * @internal
 */
export function findByKeyValue<T>(data: NonNullable<unknown>, key: string, value: string): T[] {
	/**
	...
	if (Array.isArray(data)) {
		 ...
		 */
		res = res.concat(...data.map((item) => findByKeyValue<T>(item as NonNullable<unknown>, key, value)));
	}
	// If the data is an object
		...
		for (const [, v] of Object.entries(data)) {
			res = res.concat(findByKeyValue<T>(v as NonNullable<unknown>, key, value));
		}
	}
